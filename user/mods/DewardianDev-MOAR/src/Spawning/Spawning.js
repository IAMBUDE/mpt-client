...
const buildWaves = (container) => {
    console.log(GlobalValues_1.globalValues.forcedPreset, GlobalValues_1.globalValues.currentPreset);
    ...
    for (let index = 0; index < locationList.length; index++) {
        ...
        if (map === "laboratory") {
            combinedPmcZones = combinedPmcZones.filter((zone) => zone !== "BotZoneGate1");
        }
        const bearWaves = waveBuilder(pmcCountPerSide, timeLimit, pmcWaveStart, "pmcBEAR", pmcDifficulty, true, defaultGroupMaxPMC, map === "gzHigh" ? [] : combinedPmcZones, randomBoolean ? firstHalf : secondHalf, 1, !!startingPmcs, !!morePmcGroups);
        const usecWaves = waveBuilder(pmcCountPerSide, timeLimit, pmcWaveStart, "pmcUSEC", pmcDifficulty, true, defaultGroupMaxPMC, map === "gzHigh" ? [] : combinedPmcZones, randomBoolean ? secondHalf : firstHalf, 5, !!startingPmcs, !!morePmcGroups);
        // Scavs
    ...
    // BOSS RELATED STUFF!
    const bossList = [...botConfig.bosses].filter((bossName) => !["bossZryachiy", "bossKnight", "bossBoarSniper"].includes(bossName));
    // CreateBossList
    const bosses = {};
    for (let indx = 0; indx < locationList.length; indx++) {
        ...
        if (randomRaiderGroup) {
            const raiderWave = buildBossBasedWave(randomRaiderGroupChance, "1,2,2,2,3", "pmcBot", "pmcBot", "", locationList[indx].base.EscapeTimeLimit);
            location.base.BossLocationSpawn.push(raiderWave);
        }
        if (randomRogueGroup) {
            const rogueWave = buildBossBasedWave(randomRogueGroupChance, "1,2,2,2,3", "exUsec", "exUsec", "", locationList[indx].base.EscapeTimeLimit);
    ...
    if (bossInvasion && !disableBosses) {
        ...
        for (let key = 0; key < locationList.length; key++) {
            ...
            let bossLocations = "";
            const duplicateBosses = locationList[key].base.BossLocationSpawn.filter(({ BossName, BossZone }) => {
